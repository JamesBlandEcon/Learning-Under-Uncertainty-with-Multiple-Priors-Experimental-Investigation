...
modelname = 'MixBetaLambdaConstant'
...
%02  lambda      all      all       exp(2) (set constant)
...
l = 1;
theta0 = repmat([0.5 log(2) 0 0 0 0 0 log(2) 0 0 0 0 0],[N 1]);
% 00 - Bayes in both tasks
ll{1} = @(x,l)    A_loglike_BAYES_Beta([E1_Choice_A E1_Choice_A2],[E1_SafeOption_A E1_SafeOption_A2],[x(:,1) l*ones(size(x,1),1) x(:,2:3)],[E1_NumDraws_A E1_NumDraws_A2],[E1_Draw_A E1_Draw_A2],[E1_DecisionColor_A E1_DecisionColor_A2])...
              +A_loglike_BAYES_Beta(E1_Choice_C,E1_SafeOption_C,[x(:,1) l*ones(size(x,1),1) x(:,8:9)],E1_NumDraws_C,E1_Draw_C,E1_DecisionColor_C)...
              +R_loglike2(       E1_Choice_R,E1_Prob_R,E1_SafeOption_R,[x(:,1) l*ones(size(x,1),1)]);
% 01 - A-Bayes & C-MP
ll{2} = @(x,l)    A_loglike_BAYES_Beta([E1_Choice_A E1_Choice_A2],[E1_SafeOption_A E1_SafeOption_A2],[x(:,1) l*ones(size(x,1),1) x(:,2:3)],[E1_NumDraws_A E1_NumDraws_A2],[E1_Draw_A E1_Draw_A2],[E1_DecisionColor_A E1_DecisionColor_A2])...
              +A_loglike_MPbeta_black(E1_Choice_C,E1_Prob_C,E1_SafeOption_C,[x(:,1) l*ones(size(x,1),1) x(:,10:13)],E1_NumDraws_C,E1_Draw_C,E1_DecisionColor_C)...
              +R_loglike2(       E1_Choice_R,E1_Prob_R,E1_SafeOption_R,[x(:,1) l*ones(size(x,1),1)]);

% 10 - A-MP & C-Bayes
ll{3} = @(x,l)    A_loglike_MPbeta_black([E1_Choice_A E1_Choice_A2],[E1_Prob_A E1_Prob_A2],[E1_SafeOption_A E1_SafeOption_A2],[x(:,1) l*ones(size(x,1),1) x(:,4:7)],[E1_NumDraws_A E1_NumDraws_A2],[E1_Draw_A E1_Draw_A2],[E1_DecisionColor_A E1_DecisionColor_A2])...
              +   A_loglike_BAYES_Beta(E1_Choice_C,E1_SafeOption_C,[x(:,1) l*ones(size(x,1),1) x(:,8:9)],E1_NumDraws_C,E1_Draw_C,E1_DecisionColor_C)...
              +R_loglike2(       E1_Choice_R,E1_Prob_R,E1_SafeOption_R,[x(:,1) l*ones(size(x,1),1)]);
...
% 11 - MP in both tasks
ll{4} = @(x,l)    A_loglike_MPbeta_black([E1_Choice_A E1_Choice_A2],[E1_Prob_A E1_Prob_A2],[E1_SafeOption_A E1_SafeOption_A2],[x(:,1) l*ones(size(x,1),1) x(:,4:7)],[E1_NumDraws_A E1_NumDraws_A2],[E1_Draw_A E1_Draw_A2],[E1_DecisionColor_A E1_DecisionColor_A2])...
              +   A_loglike_MPbeta_black(E1_Choice_C,E1_Prob_C,E1_SafeOption_C,[x(:,1) l*ones(size(x,1),1) x(:,10:13)],E1_NumDraws_C,E1_Draw_C,E1_DecisionColor_C)...
              +R_loglike2(       E1_Choice_R,E1_Prob_R,E1_SafeOption_R,[x(:,1) l*ones(size(x,1),1)]);
...
priorMix = ones(1,numModels); % dirichlet prior, (ones==>multivariate uniform)
 ...
 Prior_W = numParams+2;
 Prior_lambda = [2 1];                      % lognormal prior
...
%         lt = ll{tt}(T(:,:,tt),lambda)+logmvnpdf(T(:,:,tt),ThetaM,ThetaV)';
%         lp = ll{tt}(prop(:,:,tt),lambda)+logmvnpdf(prop(:,:,tt),ThetaM,ThetaV)';
...
%     end
    for tt = 1:numModels % CAN I PARALELLIZE THIS STEP? it is the slowest!!!
        lt = ll{tt}(T(:,:,tt),lambda)+logmvnpdf(T(:,:,tt),ThetaM,ThetaV)';
    for mm = 1:MH
        prop = T(:,:,tt) + cMH.*(chol(ThetaV)'*randn(size(Theta')))';
        update = zeros(N,1,numModels);
    
        
        lp = ll{tt}(prop,lambda)+logmvnpdf(prop,ThetaM,ThetaV)';
        update(:,1,tt) = (lp-lt)>log(rand(N,1));
        T(update(:,1,tt)==1,:,tt) = prop(update(:,1,tt)==1,:);
        lt(update(:,1,tt)==1)=lp(update(:,1,tt)==1);
        % check for NaNs. Kill the program if it does
        if sum(isnan(lp))>0
            fprintf('ERROR: NaNs in the likelihoods. Banish them before believing the results\n')
            kill
    ...
    for tt = 1:numModels
        typePost(:,tt) = ll{tt}(T(:,:,tt),lambda)+log(MixProbs(tt));
    end
    ...
    f = @(l) sum(sum([ll{1}(Theta,l) ll{2}(Theta,l) ll{3}(Theta,l) ll{4}(Theta,l)].*(repmat(typeI,[1 4])==repmat(1:4,[N 1])),1),2)+log(normpdf(l,Prior_lambda(1),Prior_lambda(2)));
     l_t = f(lambda);
    for mm = 1:MH
        lambda_p = lambda+0.1*randn(1,1);
        l_p = f(lambda_p);
        accept = (l_p-l_t)>log(rand(1,1));
        lambda = lambda*(1-accept)+lambda_p*accept;
        l_t = l_t*(1-accept)+l_p*accept;
    end
